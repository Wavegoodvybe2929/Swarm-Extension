...
export class RecentAnalysisProvider implements vscode.TreeDataProvider<AnalysisItem> {
    ...
    async getChildren(element?: AnalysisItem): Promise<AnalysisItem[]> {
        if (!element) {
            ...
            if (task.result) {
                items.push(
                    new AnalysisItem(
                        ...
                        task,
                        undefined,
                        task.result.success ? '$(pass)' : '$(error)'
                    )
                );
            }
            ...
            return element.task.assignedAgents.map(agentId => 
                new AnalysisItem(
                    agentId,
                    vscode.TreeItemCollapsibleState.None,
                    'agent',
            ...
            // Show suggestions
            return element.task.result.suggestions.map((suggestion, index) => 
                new AnalysisItem(
                    suggestion,
                    vscode.TreeItemCollapsibleState.None,
                    'suggestion',
