...
export class SQLiteMemoryBank implements HiveMemoryBank {
    ...
    constructor(storagePath: string, maxSizeMB: number = 1024, enableCompression: boolean = true) {
    ...
    async storeAgentCreation(agent: Agent): Promise<void> {
        ...
        const transaction = this.db.transaction(() => {
            ...
            taskStmt.run(
                ...
                agent.id,
                'agent_creation',
                `Created ${agent.type} agent: ${agent.name}`,
                'completed',
    ...
    async storeTaskExecution(taskId: string, agentId: string, result: any): Promise<void> {
        ...
        const transaction = this.db.transaction(() => {
            ...
            stmt.run(
                ...
                result.description || 'Task execution',
                result.success ? 'completed' : 'failed',
                new Date().toISOString(),
                result.duration,
                result.success,
    ...
    async query(query: string, limit: number = 100): Promise<any[]> {
        ...
        for (const search of searches) {
            ...
            if (conditions) {
                ...
                results.push(...tableResults.map((row: any) => ({
                    ...row,
    ...
    async getAgentContext(agentId: string): Promise<any> {
        ...
        return {
            ...
            recentInteractions,
            performanceMetrics,
            lastActive: (recentTasks[0] as any)?.completed_at || (recentInteractions[0] as any)?.timestamp
        };
    }
    ...
    async storeNeuralPattern(pattern: {
        ...
        stmt.run(
            ...
            JSON.stringify(pattern.contextTags)
        );
    }

    async getNeuralPatterns(type?: string, limit: number = 50): Promise<any[]> {
        this.ensureInitialized();
    ...
    async getCodePatterns(language?: string, type?: string, limit: number = 50): Promise<any[]> {
        this.ensureInitialized();
    ...
    async storeErrorPattern(error: {
        ...
        const existing = existingStmt.get(error.type, error.message) as { id: number; occurrence_count: number } | undefined;
    ...
    async getErrorPatterns(type?: string, severity?: string, limit: number = 50): Promise<any[]> {
        this.ensureInitialized();
    ...
    async storeDecision(decision: {
        ...
        stmt.run(
            decision.context,
            decision.type,
            JSON.stringify(decision.inputFactors),
    ...
    async storeFileChange(change: {
        ...
        stmt.run(
            ...
            change.taskId || null,
            change.reason || null,
            change.diffData || null,
            change.sizeBefore || null,
    ...
    async getLatestSwarmState(): Promise<any> {
        this.ensureInitialized();
    ...
    private async migrateFromJSON(): Promise<void> {
        ...
        try {
            ...
            if (oldData.specifications) {
                for (const [id, spec] of Object.entries(oldData.specifications as any)) {
                    const specEntry = spec as any;
            ...
            // Migrate agents
            if (oldData.agents) {
                for (const [id, agentEntry] of Object.entries(oldData.agents as any)) {
                    const entry = agentEntry as any;
                    if (entry.agent) {
            ...
            if (oldData.taskExecutions) {
                for (const [id, taskEntry] of Object.entries(oldData.taskExecutions as any)) {
                    const entry = taskEntry as any;
    ...
    private calculateRelevance(row: any, searchTerms: string[]): number {
