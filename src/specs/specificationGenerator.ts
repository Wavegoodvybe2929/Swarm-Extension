...
import { SpecificationTask, TaskDefinition, AgentType } from '../types';
...
export class SpecificationGenerator {
    ...
    async generateSpecification(request: SpecGenerationRequest): Promise<SpecificationTask> {
        try {
    ...
    private buildSpecificationPrompt(request: SpecGenerationRequest): string {
...
Generate the specification now:`;
    ...
    private parseSpecificationResponse(response: string, request: SpecGenerationRequest): SpecificationTask {
        try {
    ...
    private parseTasks(tasksSection: string): TaskDefinition[] {
    ...
    private parseTaskBlock(block: string, index: number): TaskDefinition | null {
        try {
    ...
    private validateTaskType(type: string): 'design' | 'implementation' | 'testing' | 'review' | 'optimization' {
    ...
    private parseRequirements(requirementsSection: string): string[] {
    ...
    private parseArchitecture(architectureSection: string): string[] {
    ...
    private parseAcceptanceCriteria(criteriaSection: string): string[] {
    ...
    private parsePriority(priority: string): 'low' | 'medium' | 'high' | 'critical' {
    ...
    private parseDependencies(dependencies: string): string[] {
    ...
    private generateFallbackTitle(userRequest: string): string {
        // Extract key words from user request to create a title
        const words = userRequest.split(' ').slice(0, 6);
        return words.map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    }

    private validateSpecification(spec: SpecificationTask): void {
    ...
    private createFallbackSpecification(request: SpecGenerationRequest): SpecificationTask {
        const specId = `spec-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
