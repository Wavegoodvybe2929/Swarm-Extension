...
exports.FaultToleranceManager = void 0;
const vscode = __importStar(require("vscode"));
class FaultToleranceManager {
    constructor(topologyManager, loadBalancer, config) {
        this.healthChecks = new Map();
        this.circuitBreakers = new Map();
        this.recoveryActions = [];
        this.backupAgents = new Map();
    ...
    async handleAgentFailure(agentId, error, task) {
        try {
    ...
    async isolateAgent(agentId, reason) {
        try {
    ...
    async attemptRecovery(agentId, error, task) {
    ...
    async reassignTask(task, failedAgentId, error) {
        try {
    ...
    async spawnBackupAgent(failedAgentId) {
        try {
    ...
    async createBackupAgents(agents) {
    ...
    determineHealthStatus(healthCheck) {
    ...
    determineHealthStatusFromMetrics(responseTime, errorRate) {
    ...
    getRecentFailures(agentId, timeWindow) {
        const cutoff = Date.now() - timeWindow;
        return this.recoveryActions.filter(action => action.agentId === agentId &&
...
//# sourceMappingURL=faultTolerance.js.map